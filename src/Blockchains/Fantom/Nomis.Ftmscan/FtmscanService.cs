// ------------------------------------------------------------------------------------------------------
// <copyright file="FtmscanService.cs" company="Nomis">
...
using Nomis.Ftmscan.Calculators;
...
{
    /// <inheritdoc cref="IFantomScoringService"/>
    internal sealed class FtmscanService :
        BlockchainDescriptor,
        IFantomScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IFtmscanClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="FtmscanService"/>.
        /// </summary>
        /// <param name="settings"><see cref="FtmscanSettings"/>.</param>
        /// <param name="client"><see cref="IFtmscanClient"/>.</param>
        ...
        public FtmscanService(
            IOptions<FtmscanSettings> settings,
            IFtmscanClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "fantom";

        /// <inheritdoc />
        public string CoingeckoNativeTokenId => "fantom";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            decimal usdBalance = (priceData?.Price ?? 0M) * balanceWei?.ToFtm() ?? 0;
            var transactions = (await _client.GetTransactionsAsync<FtmscanAccountNormalTransactions, FtmscanAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<FtmscanAccountInternalTransactions, FtmscanAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<FtmscanAccountERC20TokenEvents, FtmscanAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = (await _client.GetTransactionsAsync<FtmscanAccountERC721TokenEvents, FtmscanAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            {
                aaveAccountDataResponse = (await _aaveService.GetAaveUserAccountDataAsync(AaveChain.Fantom, request.Address).ConfigureAwait(false)).Data;
            }

            #endregion Aave protocol

            var walletStats = new FantomStatCalculator(
                    request.Address,
